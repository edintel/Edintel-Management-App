...
const ExpenseForm = () => {
  ...
  const { graphService, periods, setExpenseReports } = useAppContext();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedDate < minDate || selectedDate > maxDate) {
        throw new Error('La fecha debe estar entre las últimas 2 semanas y hoy');
      }
      ...
      const newExpense = await graphService.createExpenseReport(expenseDataWithPeriod, formData.comprobante);
      ...
      setError(err.message || 'Error al crear el gasto. Por favor intente nuevamente.');
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500">
                  Rango permitido: {dateLimits.displayMin} - {dateLimits.displayMax}
                </p>
              </div>

              <div className="space-y-2">
                <label htmlFor="st" className="block text-sm font-medium text-gray-700">
                  ST *
                </label>
                <input
                  type="text"
