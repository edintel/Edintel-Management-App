...
export const useTicketData = ({
    ...
    return serviceTickets.filter(ticket => {
      ...
      if (selectedUsers.length > 0) {
        ...
        const isUserInvolved = selectedUsers.some(userId => {
          ...
          const isAssignedTech = ticket.technicians.some(tech => 
            tech.LookupId === userId
          );
  ...
  const technicians = useMemo(() => {
    return roles
      .filter(role => role.role === 'Tecnico' && role.employee)
      .map(role => ({
  ...
  const getAvailableSystems = useMemo(() => (siteId) => {
