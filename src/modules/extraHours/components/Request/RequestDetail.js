...
const RequestDetail = () => {
  ...
  // Calcular totales con el nuevo sistema
  const calculateDetailedTotals = () => {
    let tiempoMedio = 0;
    let tiempoDoble = 0;
    let tiempoDobleDoble = 0;

    // Aquí deberías usar la misma lógica de cálculo del RequestForm
    // Por simplicidad, usaré el método del servicio
    const totalHours = service?.calculateTotalHours(request.extrasInfo) || 0;
    
    // Para el detalle, necesitas implementar el cálculo completo
    // Por ahora retorno valores de ejemplo
    return {
      tiempoMedio: totalHours > 2 ? 2 : totalHours,
      tiempoDoble: totalHours > 2 ? totalHours - 2 : 0,
      tiempoDobleDoble: 0
    };
  };
